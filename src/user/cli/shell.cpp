...
void Shell::init_gui_mode(const char* username) {
    ...
    // build prompt: "username@zilos> " idc if it overflows if name is long
    int i = 0;
...
// execute_command: handle parsing and running the user input fr
void Shell::execute_command() {
    ...
    // split command and args; ts is crude but works
...
// run: infinite loop for keyboard input unless we exit idc
void Shell::run() {
    while (!should_exit) {
