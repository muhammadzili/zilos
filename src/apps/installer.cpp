...
void Installer::draw_progress_bar(int y, int percent) {
    ...
    VGA::set_cursor(x, y);
    VGA::set_color(VGA::entry_color(0, 7)); // Grey empty
    for(int i=0; i<bar_width; i++) VGA::putchar(' ');
    
    int filled = (percent * bar_width) / 100;
    VGA::set_cursor(x, y);
    VGA::set_color(VGA::entry_color(15, 2)); // Green filled
    for(int i=0; i<filled; i++) VGA::putchar(' ');
    
    VGA::set_color(VGA::entry_color(15, 1)); // Back to blue
    char pct[8];
    itoa(percent, pct, 10);
...
void Installer::screen_welcome() {
    VGA::clear();
    draw_box(0, 0, 80, 25, 1); // Blue background
    draw_box(0, 0, 80, 1, 7);  // Top bar grey
    draw_text(2, 0, "ZilOS System Setup", 0, 7);
    
    draw_box(15, 6, 50, 10, 7); // Light grey dialog
...
void Installer::screen_network() {
    ...
    // Fake loading animation for network discovery
    const char* spinner = "\\|/-";
    for(int i=0; i<8; i++) {
    ...
    draw_box(15, 12, 50, 3, 7); // clear animation area
    ...
    if (RTL8139::init()) {
        ...
        if (DHCP::discover()) {
            ...
            draw_text_centered(10, "DHCP Auto-Configuration Successful!", 2, 7); // Green text
            ...
            draw_text_centered(10, "DHCP Auto-Configuration Failed.", 4, 7); // Red text
            draw_text_centered(12, "You can configure it manually later via CLI.", 0, 7);
        }
    } else {
        draw_text_centered(10, "No Network Card Detected.", 4, 7); // Red text
        draw_text_centered(12, "You can configure it manually later via CLI.", 0, 7);
    }
    ...
    // Create the network configuration file payload
    VFS::create_file("network/config.txt", false);
    if (network_ok) {
        char config_buf[256] = "method=dhcp\n";
        
        char tk[16];
        int p = 12; // length of "method=dhcp\n"
...
void Installer::screen_username() {
    ...
    while (true) {
        ...
        draw_box(36, 10, 20, 1, 8); // Dark grey input box
...
void Installer::screen_step(const char* title, int start_pct, int end_pct, int delay_ms, const char* files[]) {
    ...
    for (int p = start_pct; p <= end_pct; p += 2) {
        ...
        if (files) {
            draw_box(15, 11, 50, 1, 7); // clear previous file
...
void Installer::screen_complete() {
    ...
    while (Keyboard::wait_get_char() != '\n');
    VGA::set_color(VGA::entry_color(15, 0)); // Restore CLI black and white
    VGA::clear();
}
