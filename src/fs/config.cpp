...
// save: store config as simple key=value pairs in vfs fr
bool Config::save() {
...
// load: parse key=value lines from config file idc about complex parsing
bool Config::load() {
    VFile* f = VFS::get_file(CONFIG_FILENAME);
    if (!f) return false;
    ...
    while (data[i]) {
        ...
        // apply settings to global config object
