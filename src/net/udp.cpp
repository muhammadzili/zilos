...
// bind: register a callback for a specific udp port fr
...
// send: wrap data in udp header and pass to ipv4 idc about checksum overhead
void UDP::send(const uint8_t* dst_ip, uint16_t src_port, uint16_t dst_port, const void* data, uint16_t len) {
    ...
    if (len > 1472) len = 1472; // truncate if too huge lol
    net_memcpy(packet + 8, data, len);

    // calc udp checksum (pseudo-header + udp segment) ts is required fr
    uint32_t sum = 0;
    ...
    if (hdr->checksum == 0) hdr->checksum = 0xFFFF; // per rfc 768 idc

    IPv4::send(dst_ip, IP_PROTO_UDP, packet, 8 + len);
}

// handle: process incoming udp packets and dispatch to callbacks fr
